var ctx, textctx, textCanvas, width, height;
var animation,
  lastTime = 0,
  Timesub = 0,
  DeltaTime = 0,
...
const AudioController = {
  ...
  init() {
    this.audio = document.getElementById('bgmMusic');
...
window.onload = function () {
  ...
  // Try to start audio after a user gesture
...
function drawBackground(ctx) {
  ...
  if (start > 0 && start <= 10) {
    ...
    drawString(ctx, str1, fontPos1.x, fontPos1.y, 'rgba(255,255,0,1)', fontSize, 'orbitron', 0, 0, 0);
    let str2 = '2025';
    let fontPos2 = new Vector(width / 2 - str2.length * fontSize * 0.3 + offPos.x, height / 3 + fontSize + offPos.y);
...
function CreateDisplay(id, width, height, border) {
  ...
  let style_arr = ['display: block;', 'margin: 0 auto;', 'background: #FFF;', 'padding: 0;', 'display: block;'];
  canvas.id = id;
